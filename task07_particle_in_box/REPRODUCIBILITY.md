...
Figures are rendered only from a study with a matching passing 37-check report.
...
The command constructs the analytical and numerical studies, requires all 37
scientific checks to pass, writes six data files and twelve figure files into a
...
| `validation_report.json` | machine-readable 37-check result |
| `manifest.json` | provenance, schemas, configuration and 18 content hashes |
