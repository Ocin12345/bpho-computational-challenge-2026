"""Structural, evidence and readability checks for the Task 10 slide pack."""
...
PRESENTATION_PATH = PRESENTATION_DIRECTORY / "Task10_Hydrogenic_Orbitals.pptx"
PREVIEW_PATH = (
    PRESENTATION_DIRECTORY
    / "preview"
    / "Task10_Hydrogenic_Orbitals_preview.png"
)
REPRODUCIBLE_CORE_TIMESTAMP = "2026-01-01T00:00:00Z"

ASSET_PAIRS = (
    (
        REPOSITORY_ROOT / "figures/task10/required_orbital_gallery.png",
        PRESENTATION_DIRECTORY / "images/01_required_orbital_gallery.png",
    ),
    (
        REPOSITORY_ROOT / "figures/task10/radial_and_nodal_structure.png",
        PRESENTATION_DIRECTORY / "images/02_radial_and_nodal_structure.png",
    ),
    (
        REPOSITORY_ROOT / "figures/task10/coloured_glass_density.png",
        PRESENTATION_DIRECTORY / "images/03_coloured_glass_density.png",
    ),
    (
        REPOSITORY_ROOT / "figures/task10/rendering_comparison.png",
        PRESENTATION_DIRECTORY / "images/04_rendering_comparison.png",
    ),
    (
        REPOSITORY_ROOT / "figures/task10/task10_summary.png",
        PRESENTATION_DIRECTORY / "images/05_task10_summary.png",
    ),
    (
        REPOSITORY_ROOT / "figures/task10/orbital_view_rotation_poster.png",
        PRESENTATION_DIRECTORY / "images/06_orbital_view_rotation_poster.png",
    ),
)
...
REQUIRED_XML_TEXT = (
    "FINAL SCRIPT",
    "normalized hydrogenic orbitals",
    "twenty-five real states",
    "Energy scales as Z squared over n squared",
    "camera motion, not electron motion",
    "all twenty-two checks pass",
    "Task 10 normalized hydrogenic-orbital summary",
    "S-through-G family progression",
    "22 of 22 scientific checks passing across 204 validated states",
)
...
def validate() -> None:
    ...
    with zipfile.ZipFile(PRESENTATION_PATH) as archive:
        ...
        if archive.read(media[0]) != ASSET_PAIRS[4][1].read_bytes():
            raise AssertionError("embedded image differs from validated Task 10 summary")
    ...
    if word_count != 50:
        raise AssertionError(f"final script must remain 50 words, found {word_count}")
    ...
    for markdown_path in (
        ...
        PRESENTATION_DIRECTORY / "deck-style.md",
    ):
    ...
    print(
        "Task 10 presentation validation: PASS "
        f"(1 slide, 1 embedded 4K visual, {word_count}-word final script, "
        f"{width}x{height} preview, 6 byte-identical source assets)"
    )
